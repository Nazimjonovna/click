...
class ApiHelper:
    ...
    @classmethod
    def get_extra_data(cls, transaction: ClickTransactionPR):
        extra_data = {}
        try:
    ...
    @classmethod
    def save_extra_data(cls, transaction: ClickTransactionPR, extra_data: dict):
    ...
    @classmethod
    def get_transaction(cls, transaction_id: int) -> Union[ClickTransactionPR, dict]:
        try:
            transaction = ClickTransactionPR.objects.get(id=int(transaction_id))
            return transaction
        except ClickTransactionPR.DoesNotExist:
            return {
                'error': -5001,
    ...
    def create_invoice(self):
        ...
        if isinstance(transaction, ClickTransactionPR):
            ...
            if invoice.status_code == 200:
                ...
                if _json['error_code'] == SUCCESS:
                    transaction.change_status(ClickTransactionPR.PROCESSING)
                else:
                    transaction.change_status(ClickTransactionPR.ERROR)
    ...
    def check_invoice(self):
        ...
        if isinstance(transaction, ClickTransactionPR):
            ...
            if check_invoice.status_code == 200:
                ...
                if _json['status'] > 0:
                    transaction.change_status(ClickTransactionPR.CONFIRMED)
                elif _json['status'] == -99:
                    transaction.change_status(ClickTransactionPR.CANCELED)
                elif _json['status'] < 0:
                    transaction.change_status(ClickTransactionPR.ERROR)
                transaction.message = json.dumps(_json)
                transaction.save()
    ...
    def create_card_token(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickTransactionPR):
            if transaction.status == ClickTransactionPR.CONFIRMED:
                return {
                    'error': ALREADY_PAID,
    ...
    def verify_card_token(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickTransactionPR):
            if transaction.status != ClickTransactionPR.PROCESSING:
                return {
                    'error': -5001,
