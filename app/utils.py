...
from .serializers import ClickTransactionSerilaizerPR
...
class PyClickMerchantAPIView(APIView):
    ...
    def post(self, request):
        serializer = ClickTransactionSerilaizerPR(data=request.data)
    ...
    def order_load(cls, order_id):
        if int(order_id) > 1000000000:
            return None
        transaction = get_object_or_404(ClickTransactionPR, id=int(order_id))
        return transaction
    ...
    def click_webhook_errors(cls, click_trans_id: str,
        ...
        if order.status == ClickTransactionPR.CONFIRMED:
            return {
                'error': ALREADY_PAID,
        ...
        if order.status == ClickTransactionPR.CANCELED or int(error) < 0:
            return {
    ...
    def prepare(cls, click_trans_id: str,
        ...
        order = cls.order_load(merchant_trans_id)
        if result['error'] == '0':
            order.status = ClickTransactionPR.WAITING
            order.save()
    ...
    def complete(cls, click_trans_id: str,
        ...
        if error and int(error) < 0:
            order.change_status(ClickTransactionPR.CANCELED)
        if result['error'] == SUCCESS:
            order.change_status(ClickTransactionPR.CONFIRMED)
    ...
    @staticmethod
    def generate_url(order_id, amount, phone_number=None, return_url=None):
