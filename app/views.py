...
class CreateClickTransactionView(CreateAPIView):
    serializer_class = serializers.ClickTransactionSerilaizerPR

    def post(self, request, *args, **kwargs):
        amount = request.POST.get('amount')
        phone_number = request.POST.get('phone_number')
        order = ClickTransactionPR.objects.create(amount=amount, phone_number = phone_number)
        return_url = 'http://127.0.0.1:8000/'
        url = PyClickMerchantAPIView.generate_url(order_id=order.id, amount=str(amount),phone_number = str(phone_number), return_url=return_url)
        return redirect(url)
...
class TransactionCheck(PyClickMerchantAPIView):
    ...
    def check_order(cls, order_id: str, amount: str):
        if order_id:
            try:
                order = ClickTransactionPR.objects.get(id=order_id)
    ...
    @classmethod
    def successfully_payment(cls, transaction: ClickTransactionPR):
